    ...
    <table class="w-full border-collapse border border-slate-200">
      <thead>
        <tr>
          ...
          <th 
            v-for="(status, key) in statistic.webmaster_stats[0].statuses" 
            :key="key"
            :class="[
              css.th, 
      ...
      <tbody>
        <tr>
          <td :class="css.td">{{ statistic.webmaster_stats[0].webmaster_name }}</td>
          <td :class="css.td" v-for="(status, key) in statistic.webmaster_stats[0].statuses" :key="key">
            {{ status }}
          </td>
